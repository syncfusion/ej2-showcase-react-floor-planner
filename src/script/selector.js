    ...
    getGradient(node) {
        ...
        node.style.gradient = {
            ...
            stops: [
                ...
                { color: this.getColor(this.mGradientColor), offset: 1 }
            ]
        };
    }
...
export class TextProperties {
    ...
    // Returns an array of text position options for nodes in a diagram.
    ...
    // Returns an array of text position options for connectors in a diagram.
    ...
    // Triggers a property change event with the specified property name and value if the propertyChange handler is defined.
    triggerPropertyChange(propName, propValue) {
        if (!isNullOrUndefined(this.propertyChange)) {
            this.propertyChange.call(this, { "propertyName": propName, "propertyValue": propValue });
        }
        ...
        if (!this.preventPropertyChange) {
            ...
            if (diagram) {
                ...
                if (diagram.connectors.length > 0) {
                    let selectedNodes = diagram.selectedItems.connectors;
                   // eslint-disable-next-line
